...
type RoomView struct {
	...
	// The one status the coordinator derives rather than stores, from what
	// the host's own machine is doing (D69): a match running on the host's PC
	// locks the room whether or not the host remembered to press anything.
	// Status already says so - it reads "locked_in_game" - and this says the
	// reason was the match.
